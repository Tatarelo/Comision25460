const productos = [{
    id: "1",
    nombre: "Turbo Chevrolet Cruze 1.4T",
    foto: "https://icdn.dtcn.com/image/digitaltrends_es/turbo-2-500x500.jpg",
    detail: "Turbocompresor Chevrolet Cruze 1.4T, Diametro de entrada 30mm, Numero de pieza: 12679375",
    precio: 39000,
    categoria: "inyeccion",
    stock: "3"
  },
  {
    id: "2",
    nombre: "Filtro de aire",
    foto: "https://www.motoryracing.com/images/noticias/21000/21881/1.jpg",
    detail: "Filtro de aire conico para Chevrolet Onix",
    precio: 5400,
    categoria: "filtros",
    stock: "17"
  },
  {
    id: "3",
    nombre: "Filtro de aceite",
    foto: "https://www.actualidadmotor.com/wp-content/uploads/2019/05/filtro-aceite-830x460.jpg",
    detail: "Filtro de aceite Chevrolet Cruze 1.4T de papel microporoso",
    precio: 3900,
    categoria: "filtros",
    stock: "31"
  },
  {
    id: "4",
    nombre: "Filtro de nafta",
    foto: "https://refaccionariamario.info/29033-tm_thickbox_default/filtro-de-gasolina-ac-delco-para-chevy-corsa-tornado.jpg",
    detail: "Filtro de nafta para Chevrolet Corsa 1.4 8V para vehiculos a nafta o GNC",
    precio: 3700,
    categoria: "filtros",
    stock: "23"
  },
  {
    id: "5",
    nombre: "Filtro de habitaculo",
    foto: "http://d2r9epyceweg5n.cloudfront.net/stores/994/749/products/akx-357231-75152d9bfdc3373fc915595823388175-640-0.png",
    detail: "Filtro de habitaculo Chevrolet Cruze 1.4T",
    precio: 2400,
    categoria: "filtros",
    stock: "15"
  },
  {
    id: "6",
    nombre: "Llanta Cruze 2017",
    foto: "https://content.ib2c.com.ar/306/products/7614795131742.jpg",
    detail: "Llanta Chevrolet Cruze Premier 1.4T 2017, centro de rueda 300mm, diametro de llanta: 17 pulgadas, centro de tuercas: 4x100",
    precio: 25000,
    categoria: "llantas",
    stock: "4"
  },
  {
    id: "7",
    nombre: "Llanta Ford Mustang",
    foto: "https://www.autoclasico.es/files/imgs/mini/llanta-100-classic-cromo-17x7-ford-mustang-1964-1973-10607.png",
    detail: "Llanta Ford Mustang, centro de rueda 320mm, diametro de llanta: 18 pulgadas, centro de tuercas: 5x108",
    precio: 61000,
    categoria: "llantas",
    stock: "9"
  },
  {
    id: "8",
    nombre: "Cuerpos de admision",
    foto: "https://m.media-amazon.com/images/I/71rrynSItpL._AC_SL1500_.jpg",
    detail: "Compatible con motores GM LS1, LS2, LS6 | Aluminio negro | Acepta cuerpos de acelerador de 3.622 in/4.016 in | Rango de 2200-6800 RPM | Incluye kit de riel de combustible, sellos de admisión, hardware",
    precio: 120000,
    categoria: "inyeccion",
    stock: "3"
  }
]

export const getFetch = new Promise((resolve) => {
  setTimeout(() => {
    resolve(productos)
  }, 3000)
})

export const getFetchProd = new Promise((resolve) => {
  setTimeout(() => {
    resolve(productos)
  }, 3000)
})